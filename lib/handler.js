...
// Create Account
handler.accountCreat = function (data, callback) {
  // Reject any request that isn't a GET
  if (data.method == 'get') {
    // Prepare data for interpolation
    var templateData = {
...
handler.public = function (data, callback) {
  ...
  if (data.method == 'get') {
    ...
    let it = data.trimmedPath.split("/");
    let trimmedAssetName = it[it.length - 1];
    // var trimmedAssetName = data.trimmedPath.replace('public/','').trim();
    // console.log(trimmedAssetName,"trimiing");
...
handler._checks.post = function (data, callBack) {
  ...
  console.log(protocol, succesCodes, timeoutSecods);
